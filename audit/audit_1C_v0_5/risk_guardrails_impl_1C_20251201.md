...
## Backlog 1D+ — Guardrails y mejoras futuras

Esta sección describe las extensiones previstas para fases posteriores (1D+), que deberán construirse sobre la base establecida en 1C.

### 1. Guardrails de volatilidad

**Objetivo:** complementar el guardrail de DD global y el stop ATR con límites basados en volatilidad reciente.

Líneas generales:

- Introducir un **volatility_stop** basado en:
  - Desviación estándar rolling (σ rolling) de los retornos.
  - Porcentaje de ATR sobre el precio (p.ej. múltiplos de ATR).
- Usos previstos:
  - Reducir tamaño de posición o bloquear nuevas entradas en entornos de volatilidad anómala.
  - Ajustar dinámicamente stops o límites de riesgo cuando σ/ATR se disparan.
- Requisitos:
  - Definir parámetros de ventana rolling y umbrales en `risk_rules.yaml`.
  - Asegurar coherencia con el guardrail de DD (no duplicar lógica, sino complementarla).

### 2. Reglas de liquidez

**Objetivo:** evitar concentraciones de riesgo en activos con poca liquidez.

Líneas generales:

- Medir liquidez a partir de:
  - **ADV** (Average Daily Volume).
  - Porcentaje del volumen diario que puede tomar la estrategia sin impactar en exceso el mercado.
- Posibles reglas:
  - Límite de tamaño máximo por activo según ADV.
  - Bloqueo de señales en activos que caen por debajo de un umbral mínimo de liquidez.
- Configuración:
  - Nuevos campos en `risk_rules.yaml` para:
    - `max_notional_per_liquidity_bucket`
    - `min_liquidity_threshold`
  - Posible segmentación por clases de activo o mercados.

### 3. Overrides por activo / estrategia

**Objetivo:** permitir ajustes finos sin romper la capa de guardrails globales.

Líneas generales:

- Introducir una capa de **per-asset / per-strategy overrides**:
  - Modificadores de límites de DD local.
  - Ajustes de fracción de Kelly o tamaño máximo de posición.
  - Configuración de stops específicos (p.ej. ATR más laxo o más estricto).
- Principios:
  - Los overrides nunca pueden desactivar los guardrails globales, solo hacerlos más conservadores o adaptados al contexto.
  - Deben ser auditables y quedar totalmente trazados en configuración y logs.

### 4. Integración con `stress_tester` y métricas de portfolio

**Objetivo:** validar los guardrails no solo a nivel unitario, sino de portfolio completo bajo escenarios de estrés.

Líneas generales:

- Integrar los guardrails DD / ATR / volatilidad / liquidez con:
  - Escenarios de estrés sintéticos (shocks de precios, gaps, volatilidad extrema).
  - Monte Carlo de trayectorias de PnL / equity.
- Métricas a monitorizar:
  - **Calmar ratio**, **max drawdown**, **volatilidad de la curva de equity**.
  - Porcentaje de tiempo con guardrails activos (DD, ATR, volatilidad, liquidez).
- Deliverables previstos:
  - Extensión de `stress_tester` para simular activación de guardrails.
  - Nuevos reportes en `report/` con análisis de sensibilidad y escenarios.
